...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Cr=floor(matrizC(1:9,:)*255); %simplificado
%Cra=floor(matrizCa(1:9,:)*255); %completo
%clear matrizCa;
matrizI=[];
for i=1:size(matrizCa,2)
unico=true;
for j=1:size(matrizC,2)
if isequal(matrizCa(:,i),matrizC(:,j))
matrizI=[matrizI j];
break;
endif
endfor
endfor
%[Matriz Completa; Matriz reducida unica]
index=[1:size(matrizI,2); matrizI]';